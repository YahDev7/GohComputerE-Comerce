...
const Header = () => {
    ...
    const { itemsCarr, statesidebarCarr, statesetSidebarCarr, pluscarr, minuscarr, btnremovepro, subtotal, loaderCarrito } = useContext(CarritoContext);
    const { stateToken, setStateToken,stateDataUserToken } = useContext(TokenContext)
    const { stateCategorias } = useContext(CategoriaContext)
    const { searchnav, procarr } = useHeader(navigate, itemsCarr, stateCategorias, pluscarr, minuscarr, btnremovepro, subtotal);
    const logout = () => {
    ...
    return (
        <>
            <header id="Header" className="!bg-blue-950" style={{ height: "150px", transition: "300ms all" }}>
                            ...
                            <div className="max-sm:col-start-2 max-sm:col-end-6  col-start-3 col-end-6">
                                <form onSubmit={(e) => {
                                    ...
                                    <div className="input-group mb-3">
                                        <input className="form-control !rounded-xl  w-50" name="search" type="search" placeholder="Search" aria-label="Search" />
                                        <button className="btn bg-none border-spacing-0 outline-0  text-white !rounded-xl right-1" > <i className="fas fa-search"></i></button>
                                    </div>
                                </form>
                            </div>
                            <div className=" col-start-7 grid grid-cols-2 justify-items-end  gap-4">

                                <li className="  nav-item dropdown !text-white p-0" style={{ listStyle: "none" }}>
                                    <a className="nav-link dropdown-toggle text-white" href="#" id="navbarScrollingDropdown" role="button" data-bs-toggle="dropdown" aria-expanded="false">
                                        <i className="fas fa-user">  {stateDataUserToken&&stateDataUserToken.nombre}</i>
                                    </a>
                                    <ul className="dropdown-menu mt-2 !bg-blue-950" aria-labelledby="navbarScrollingDropdown" >
                                        {/*  <li><a className="dropdown-item !bg-blue-950 text-white hover:bg-red" href="#">Perfil</a></li> */}
                                        {/*  <li><a className="dropdown-item !bg-blue-950 text-white" href="#">Ajustes</a></li> */}
                                        {!stateToken ?

                                            <li>

                                                <a className="nav-link iconCarr text-white" href="#/login" > Iniciar Sesion</a>


                                            </li>
                                            :
                                            <div>
                                                <li><a className="dropdown-item !bg-blue-950 text-white">{stateDataUserToken&&stateDataUserToken.email}</a></li>
                                                <li><a className="dropdown-item !bg-blue-950 text-white" href="#/pedidos">Mis pedidos</a></li>
                                                <li><hr className="dropdown-divider text-white" /></li>
                                                <a className="ps-3 text-white cursor-pointer" onClick={(e) => logout()}>Cerrar Sesión</a>
                                            </div>
             ...
             {/*    <div className="text-center !bg-blue-950 text-white p-1">
                ...
                {/*  <div className="offcanvas offcanvas-start" data-bs-scroll="true" id="menuLateral" aria-labelledby="offcanvasNavbarLabel">
                    ...
                    <div className="menuLateralBody offcanvas-body">
                        <ul className="navbar-nav accordion accordion-flush menuLateralUl " id="MenuLateralAccordionDnone">
                            ...
                            {
                                stateCategorias.map((el,index) => <li className="accordion-item " key={el._id}>

                                    <a href={"#/subcategorias/" + el._id} className="nav-link menuLateralCateAcordeon" id="flush-acordionOne" > {el.nombre}</a>
                                    <i className="fas fa-plus fs-4 subCatBtn" onClick={() => toogleAcordeon(index)} ></i>

                                   {
                                     <div>
                                      {subcatsBox()}

                                  {StatesubCats.length>=1&&
                                     
                                         StatesubCats.map(el2 => <div className="ps-5" key={el2._id} >
                                             <a href={"#/productos/" + el2._id}>
                                                 {el2.nombre}
