...
export const comprobanteFetch={    
    get: async(token)=> {
        let headers={ Authorization: `Bearer ${token}`,}
        let res= await method.get(BaseURLAPI2+ROUTES_BACK.ADMIN.COMPROBANTE.GET,headers)
        return res
    }, 
    getOne: async(id,token)=> {
        let headers={ Authorization: `Bearer ${token}`,}
        let res= await method.get(BaseURLAPI2+ROUTES_BACK.ADMIN.COMPROBANTE.GETID(id),headers)
        return res
    },
    /* getOneGuia: async(id,token)=> {
        let headers={ Authorization: `Bearer ${token}`,}
        let res= await method.get(BaseURLAPI2+ROUTES_BACK.ADMIN.COMPROBANTE.GETIDGUIA(id),headers)
        return res
    }, */
    ...
    post: async(token,body)=> {
        ...
        let res= await method.post(BaseURLAPI2+ROUTES_BACK.ADMIN.COMPROBANTE.POST,body,headers)
        return res
    },
    ...
    put: async(id,body,token)=> {
        let headers={ Authorization: `Bearer ${token}`, "Content-Type": "application/json"}
        let res= await method.put(BaseURLAPI2+ROUTES_BACK.ADMIN.COMPROBANTE.PUT(id),body,headers)
        return res
    }, 
    deleteGuia: async(id,token)=> { //IDENTIFICAR EL ERROR
        let headers={ Authorization: `Bearer ${token}`}
        let res= await method.delete(BaseURLAPI2+ROUTES_BACK.ADMIN.COMPROBANTE.DELETE(id),headers)
        return res
    }, 
