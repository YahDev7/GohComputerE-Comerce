...
export const CategoriaFetch={    
    get: async(token)=> {
        let headers={ Authorization: `Bearer ${token}`,}
        let res= await method.get(BaseURLAPI2+ROUTES_BACK.ADMIN.CATEGORIA.GET,headers)
        return res
    }, 
    ...
    post: async(token,body)=> {
        ...
        let res= await method.post(BaseURLAPI2+ROUTES_BACK.ADMIN.CATEGORIA.POST,body,headers)
        return res
    }, 
